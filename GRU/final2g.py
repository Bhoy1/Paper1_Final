...
#7/9 evening, trying to match the paper, went to their code and changed some NN architecture 
#and parameters to make it match according to the code 
...
class Config:
    ...
    update_epochs: int = 4      # Same as sgd_passes (just rename)
    num_minibatches: int = 4     # Direct control over mini-batch count
    ...
    # Architecture parameters (matching PopGym)
    encoder_dim: int = 256         # Dimension of encoded observations (PopGym's 'h')
    hidden_size: int = 256         # GRU hidden state size (PopGym's 'h_memory')
...
#need to relook at whenever I try it again 
...
class PPOGRUPolicy(nn.Module):
    ...
    def __init__(self, obs_space: gym.Space, action_space: gspaces.Space):
        ...
        if img_like:
            ...
            self.encoder = DenseEncoder(obs_space, emb_dim=self.encoder_dim)
        ...
        self.gru = nn.GRU(self.encoder.out_dim, self.hidden_size, batch_first=True)
        ...
        # This overwrites the orthogonal init for just the final layers
        if isinstance(action_space, gspaces.Discrete):
    ...
    def forward(
        ...
        # ADD THIS: Apply the extra preprocessor
        # Need to handle the shape properly
        if x.dim() == 3:  # (B, T, emb_dim)
            B, T, F = x.shape
            x_flat = x.reshape(B * T, F)
            x_flat = self.preprocessor(x_flat)
...
def train(
    ...
    for ep in range(epochs):
        ...
        for update_epoch in range(config.update_epochs):  # Just rename sgd_passes
