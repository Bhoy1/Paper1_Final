...
#7_20, changed configs to old to try and get it to work again, this is on MiniGrid 
...
class Config:
    ...
    update_epochs: int = 4      # Number of epochs over collected data
    num_minibatches: int = 4    # Number of mini-batches per epoch
    ...
    USE_CUDA: bool = False          # set to False to force CPU
    ...
    use_belief_warmup = False #warmup to fix cold start problem 
...
class PPOSurpriseGRUPolicy(nn.Module):
    ...
    def step(self, x_enc, h_prev):
        ...
        h_new = self.sgru(x_enc, h_prev, S_vec, epoch=self.current_epoch)  # ADD epoch param
    ...
    def forward(
        ...
        if x.dim() == 3:  # (B, T, emb_dim)
            ...
            S_ts = []
            for t in range(T):
                logits_t, value_t, h, S_t = self.step(x[:, t], h.squeeze(0) if h.dim() == 3 else h)
            ...
            return logits, values, h.unsqueeze(0) if h.dim() == 2 else h, S_t
            ...
            logits, value, h_new, S_t = self.step(x, h)
            ...
            return logits, value, h_new.unsqueeze(0), S_t
    ...
    def act(self, obs, h, deterministic=False):
        ...
        logits, v, h1, S_t = self.forward(obs, h)
    ...
    def evaluate_actions(self, obs, h, actions):
        ...
        logits, v, _, S_t = self.forward(obs, h)
        ...
        return v, logp, entropy, S_t  # Added S_t return
    ...
    for ep in range(epochs):
        ...
        for t in range(steps):
            ...
            if done or t == steps - 1:
                h = h.detach()
                _, last_val, _, _ = model(obs, h)  # Modified to handle 4 outputs
        ...
        for i in range(buf.ptr):
            if buf.done[i] or i == buf.ptr - 1:
                ...
                if i < buf.ptr - 1:  # Don't update start at the very end
                    start = i + 1
        ...
        # Calculate episodes per mini-batch
        ...
        for update_epoch in range(config.update_epochs):
            # Shuffle episode order (not timesteps!)
            ...
            for mb_start in range(0, len(shuffled_episodes), episodes_per_minibatch):
                ...
                for ep_idx in mb_episode_indices:
                    ...
                    i = ep_start
                    while i < ep_end:
                        j = min(i + config.tbptt_len, ep_end)
                        ...
                        # Check if this is an image observation space
                        is_image = (isinstance(buf.obs_space, gspaces.Box) and 
                                    len(buf.obs_space.shape) == 3 and 
                                    buf.obs_space.shape[-1] == 3)

                        if is_image:
                            obs_seq = obs_slice.unsqueeze(0)
                        elif isinstance(buf.obs_space, gspaces.Discrete):
                            obs_seq = obs_slice.unsqueeze(0)
                        ...
                        # Use the stored initial hidden state for this episode
                        h0 = episode_hidden_states[ep_start].unsqueeze(0)
                        
                        # Evaluate actions
                        val, logp, entropy, S_pred = model.evaluate_actions(obs_seq, h0, act_seq)
                        ...
                        loss = (loss_pi + 
                               ...
                               effective_belief_coef * loss_belief - 
        ...
        if log_path is not None:
            with log_path.open("a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    np.mean(belief_losses),  # Added belief loss to logging
...
def setup_logger(env_id: str, config: Config, model_tag: str, trial_num: Optional[int] = None) -> Path:
    ...
    with path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([f"{k}={v}" for k, v in config.__dict__.items()])
        writer.writerow([
            "epoch", "mean_return", "mean_length", "norm_length", "success",
            "pi_loss", "v_loss", "entropy", "belief_loss", "best_mmer", "best_epoch"
        ])
    return path
...
def run_multiple_trials(env_id: str, env_fn, model_class, model_kwargs,
    ...
    for trial in range(num_trials):
        ...
        eval_env = env_fn(env_id, render=True)
